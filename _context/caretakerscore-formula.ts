 ...
 * - Preserve Special Phase gate: caretakerScore >= 95. (handled outside this function)
 *
 * Inputs:
...
export type CaretakerScoreConfig = {
  ...
  // - fitnessRatioPass maps to 60
  // - fitnessRatioExcellent maps to 100
  // - below pass scales up from 0..60, above pass scales up from 60..100
  fitnessCurve: {
...
export function computeCaretakerScore(
  ...
  // Weighted average in 0..100 space (no TotalTicks division)
  const raw =
    w.mood * mood +
...
/**
 * Letter grade helper (Common Letter Grade Scale).
 * If you want +/- grades, extend this.
 */
 ...
 * - At end of Adult phase, if caretakerScore >= 95 => Special Phase.
 */
